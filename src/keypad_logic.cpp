...
static void create_default_mqtt_config(String full_topic_format,
                                       String device_name, String& protocol)
{
    // Note that '#' is generally not allowed as a topic because it is a
    // wildcard
    int row, col;
    char key;
    char key_s[2] = { '\0', '\0' };
    ...
    for (row = 0; row < ROWS; row++) {
        for (col = 0; col < COLS; col++) {
            ...
            full_command
                = mqtt_construct_topic(full_topic_format, "cmnd", topic, key_s);
...
static bool get_json_config(String http_server, uint16_t http_port,
                            String http_user, String http_password,
                            String http_uri, String mac_addr,
                            String& device_name, String& protocol,
...
#else // FETCH_CONFIG
    uint32_t i;
    String mac_addr_f(mac_addr);
    String url;
    int http_code;
    String payload;

    mac_addr_f.replace(":", "-");
    url = http_uri + mac_addr_f + ".json";
    dbg_printf("Http json URL: %s\n", (const char*)url.c_str());

    payload = http_get(http_server, http_port, http_user, http_password, url,
                       http_code); // Specify request destination
    if (http_code <= 0) {
        ...
        device_name = String((const char*)jsonRoot["device"]);
        protocol = String((const char*)jsonRoot["protocol"]);
        if (protocol == "MQTT") {
            full_topic_format
                = String((const char*)jsonRoot["full-topic-format"]);
        }

        dbg_printf("Protocol = \"%s\", Full topic format = \"%s\"\n",
                   protocol.c_str(), full_topic_format.c_str());
        JsonObject buttons = jsonRoot["buttons"];
        i = 0;
        for (JsonObject::iterator it = buttons.begin(); it != buttons.end();
             ++it) {
            i++;
            ...
            if (protocol == "MQTT") {
                ...
                full_command = mqtt_construct_topic(full_topic_format, "cmnd",
                                                    topic, cmnd);
                dbg_printf("key: \"%c\", device: \"%s\", topic: \"%s\", cmnd: "
                           "\"%s\", \"%s\"\n",
...
{
    bool config_done;
    config_done = get_json_config(http_server, http_port, http_user,
                                  http_password, http_uri, mac_addr,
                                  device_name, protocol, full_topic_format);

    if (!config_done) {
        dbg_printf("Failed to fetch config file for device \"%s\"\n",
                   mac_addr.c_str());
        create_default_mqtt_config(full_topic_format, device_name, protocol);
    }
}
