...
void setup()
{
  ...
  setup_keypad(http_server, http_port, http_user, http_password, http_uri, mac_addr, device_name, protocol);
  if (protocol == "MQTT") {
    setup_mqtt(device_name, MQTT_SERVER, MQTT_PORT, MQTT_USER, MQTT_PASSWORD);
  }
  else if (protocol == "Domoticz") {
    domoticz_setup(DOMOTICZ_SERVER, DOMOTICZ_PORT, DOMOTICZ_USER, DOMOTICZ_PASSWORD);
  }
  else {
